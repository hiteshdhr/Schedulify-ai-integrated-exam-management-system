  ...
  const onSubmit = async (data: SignupForm) => {
    try {
      ...
      if (!response.ok) {
        ...
        if (message.toLowerCase().includes('email') || message.toLowerCase().includes('user already exists')) {
      ...
      toast.error('Signup failed. Could not connect to the server.');
    ...
    <Card className="w-full shadow-2xl border-0 bg-white/80 backdrop-blur">
      <CardHeader className="space-y-4 text-center pb-8">
        ...
        <CardDescription className="text-lg">
          Join thousands of students organizing their academic life with Schedulify.
        </CardDescription>
      </CardHeader>
